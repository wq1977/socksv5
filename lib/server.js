    ...
    function accept(intercept) {
      ...
      if (socket.writable) {
        if (intercept) {
          ...
          proxySocket.bind(self)(socket, reqInfo);
      }
    }
    function deny() {
      if (handled)
...
function ssProxySocket(socket, req) {
  const {encode, decode} = this._options
  ...
  this._onData = () => {
    dstSock.removeListener('data', this._onData)
    if (connected) {
      return
    }
    //now we know that server is ready
     connected = true;
     if (socket.writable) {
  ...
  dstSock.on('error', onError)
         .on('connect', function() {
           ...
           console.log(reqjson)
           const buffer = Buffer.from(reqjson)
           dstSock.write(encode(buffer))
         })
         .on('data', this._onData)
         .connect(this._options.ssport, this._options.ssserver);
  socket.dstSock = dstSock;
}

function proxySocket(socket, req) {
  if (this._options.ssserver && this._options.ssport) {
    ssProxySocket.bind(this)(socket, req)
    return
  }
    ...
    dstSock.on('error', onError)
           .on('connect', function() {
             ...
             if (socket.writable) {
              ...
              socket.write(bufrep);
              socket.pipe.pipe(dstSock).pipe(socket);
              socket.resume();
             } else if (dstSock.writable)
              dstSock.end();
           })
